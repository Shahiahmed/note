...
- **Установка на телефон** — приложение ставится на домашний экран и открывается
  на весь экран, без адресной строки. На узком экране разделы переезжают в нижнюю панель.
...
Приложение — PWA, поэтому ставится прямо из браузера, без магазина приложений.
Нужен HTTPS: на Vercel всё работает, а вот открытый по локальной сети `http://192.168.…`
установить не даст — браузер такое приложением не считает.

- **Android, Chrome** — откройте сайт, меню `⋮` → «Установить приложение».
- **iPhone, Safari** — кнопка «Поделиться» → «На экран „Домой"». Именно Safari:
  Chrome на iOS ставить приложения не умеет.

Под иконкой будет подпись «Кабинет». Название намеренно нейтральное: манифест обязан
открываться без входа, иначе браузер не предложит установку, — значит, его может
прочитать посторонний. Поменять можно в `app/manifest.ts` (`name` и `short_name`)
и в `appleWebApp.title` в `app/layout.tsx`.

**Офлайн приложение не работает, и это сделано намеренно.** Service worker кеширует
только код страниц и иконки; ответы API и HTML в кеш не попадают. Иначе выписки
по деньгам оставались бы на телефоне и после выхода из аккаунта.
...
app/
  ...
  icon.png,
  apple-icon.png        иконки приложения
  api/
    auth/               login, logout
...
public/
  sw.js                 service worker: кеширует только статику
